...
intrinsic M3A(:base_ring := ZZ) -> AlgGen, SetIndx, AlgMatElt
  {
  The 3A(al, bt) algebra over a function field, with generators and its Frobenius form.  Optional paramenter base_ring of the base ring.
  }
...
intrinsic M4Y_bt(bt::RngElt) -> AlgGen, SetIndx, AlgMatElt
  ...
  A, gens, frob := M4Y_bt(:base_ring:=F);
...
intrinsic M4Y_al(al::RngElt) -> AlgGen, SetIndx, AlgMatElt
  ...
  A, gens, frob := M4Y_al(:base_ring:=F);
...
intrinsic M6Y(: base_ring := QQ) -> AlgGen, SetIndx, AlgMatElt
  {
