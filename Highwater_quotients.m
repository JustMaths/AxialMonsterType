...
intrinsic HighwaterQuotient(S::SeqEnum) -> AlgGen, SetIndx, AlgMatElt
  {
  Returns the quotient of the Highwater algebra by the ideal generated by s_1 a_1 + ... + s_n a_n, where S = [s_1, ..., s_n], and its generators and its Frobenius form.
  }
  n := #S;
  require n ge 3: "The sequence must have length at least 3";
  require not IsZero(S[1]): "The start of the sequence must be non-zero";
  
  base_ring := Universe(S);
  require ISA(Type(base_ring), Rng): "The sequence must be a sequence of ring or field elements";
  F := FieldOfFractions(base_ring);
  require Characteristic(F) notin {2,3}: "The characteristic must not be 2 or 3";
  ...
  // Check if there could be a quotient in the cover and display a warning if  so  
  al0_bar := &+[ S[i] : i in [1..n] | i mod 3 eq 1];  // NB the sequence should be numbered starting from 0, not 1
  al1_bar := &+[ S[i] : i in [1..n] | i mod 3 eq 2];
  al2_bar := n eq 2 select 0 else &+[ S[i] : i in [1..n] | i mod 3 eq 0];
  assert al0_bar + al1_bar + al2_bar eq 0;
  
  if Characteristic(base_ring) eq 5 and al0_bar eq 0 and al1_bar eq 0 then
    print "Warning: the Highwater algebra has a cover in characteristic 5.  If you want this instead, please use HighwaterCoverQuotient.\n";
  end if;
...
intrinsic HighwaterCoverQuotient(L::SeqEnum) -> AlgGen, SetIndx, AlgMatElt
  {
  Returns the quotient of the cover of the Highwater algebra by the ideal generated by al_1 a_1 + ... + al_n a_n, where L = [al_1, ..., al_n], and its generators and its Frobenius form.
  }
  n := #L;
  require n ge 3: "The sequence must have length at least 3";
  require not IsZero(L[1]): "The start of the sequence must be non-zero";
    
  base_ring := Universe(L);
  require ISA(Type(base_ring), Rng): "The sequence must be a sequence of ring or field elements";
  F := FieldOfFractions(base_ring);
  require Characteristic(F) notin {2,3}: "The characteristic must not be 2 or 3";
